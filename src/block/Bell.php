...
final class Bell extends Transparent{
	...
	protected function recalculateCollisionBoxes() : array{
		if($this->attachmentType === BellAttachmentType::FLOOR){
			return [
				AxisAlignedBB::one()->squashedCopy($this->facing->axis(), 1 / 4)->trimmedCopy(Facing::UP, 3 / 16)
			];
		}
		...
		$box = AxisAlignedBB::one()
			->squashedCopy($this->facing->rotateY(true)->axis(), 1 / 4)
			->trimmedCopy(Facing::UP, 1 / 16)
			->trimmedCopy(Facing::DOWN, 1 / 4);

		return [
			$this->attachmentType === BellAttachmentType::ONE_WALL ? $box->trimmedCopy($this->facing, 3 / 16) : $box
		];
	}
	...
	public function place(BlockTransaction $tx, Item $item, Block $blockReplace, Block $blockClicked, Facing $face, Vector3 $clickVector, ?Player $player = null) : bool{
		...
		if($face === Facing::UP){
			if($player !== null){
				$this->setFacing($player->getHorizontalFacing()->opposite());
			}
	...
	public function onNearbyBlockChange() : void{
		foreach(match ($this->attachmentType) {
			...
			BellAttachmentType::ONE_WALL => [$this->facing->opposite()],
			BellAttachmentType::TWO_WALLS => [$this->facing, $this->facing->opposite()]
	...
	public function onInteract(Item $item, Facing $face, Vector3 $clickVector, ?Player $player = null, array &$returnedItems = []) : bool{
		if($player !== null){
			$faceHit = $player->getHorizontalFacing()->opposite();
	...
	public function onProjectileHit(Projectile $projectile, RayTraceResult $hitResult) : void{
		$faceHit = $projectile->getHorizontalFacing()->opposite();
		if($this->isValidFaceToRing($faceHit)){
			$this->ring($faceHit);
		}
	}
	...
	private function isValidFaceToRing(Facing $faceHit) : bool{
		return match ($this->attachmentType) {
			...
			BellAttachmentType::FLOOR => $faceHit->axis() === $this->facing->axis(),
			BellAttachmentType::ONE_WALL, BellAttachmentType::TWO_WALLS => $faceHit === $this->facing->rotateY(false) || $faceHit === $this->facing->rotateY(true),
		};
	}
}
