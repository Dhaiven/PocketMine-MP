...
class Vine extends Flowable{
	...
	public function place(BlockTransaction $tx, Item $item, Block $blockReplace, Block $blockClicked, Facing $face, Vector3 $clickVector, ?Player $player = null) : bool{
		if(!$blockReplace->getSide($face->opposite())->isFullCube() || $face->axis() === Axis::Y){
			return false;
		}

		$this->faces = $blockReplace instanceof Vine ? $blockReplace->faces : [];
		$this->faces[$face->opposite()->name] = $face->opposite();
	...
	public function onNearbyBlockChange() : void{
		...
		foreach($this->faces as $face){
			if(!isset($supportedFaces[$face->name]) && !$this->getSide($face)->isSolid()){
				unset($this->faces[$face->name]);
				$changed = true;
			}
		}
